...
export class Pokemon {
	...
	*/
	...
	ignoringAbility() {
		...
		if (this.abilityState.ending) return true;
		...
		const neutralizinggas = this.battle.field.pseudoWeather['neutralizinggas'];
		if (!neutralizinggas) return false;
		const pokemon = neutralizinggas?.source as Pokemon;
		if (!pokemon.transformed && !pokemon.abilityState.ending && !this.volatiles['commanding']) {
			return true;
		}
